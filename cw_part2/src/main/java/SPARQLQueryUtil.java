...
public class SPARQLQueryUtil {
    private Model model;
    private String prefix = ""; // 可选，或已有设置

    // ✅ 原有构造器，可能是这样（路径方式加载）
    public SPARQLQueryUtil(String ontologyPath) {
        this.model = RDFDataMgr.loadModel(ontologyPath);
    }

    // ✅ 你要添加的新构造器：支持传入已有的 Model（例如 reasonedModel）
    public SPARQLQueryUtil(Model model) {
        this.model = model;
    }

    // 🛠 其他方法，例如 executeQuery() 等
    public void executeQuery(String queryString, String outputCSVPath) {
        // 执行 SPARQL 查询逻辑
    }
}
